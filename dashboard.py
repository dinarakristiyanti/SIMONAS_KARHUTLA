...
def get_dashboard():
    ...
    st.markdown(f'''
        ...
        <div class='centered-text body'>
            <div class='title-container'>
                ...
                <h1 style='text-align:center; margin-bottom:10px;'>KARHUTLA: Sentiment Analysis Dashboard</h1>
            </div>
        </div>
    ...
    with body2:
        ...
        def display_emoticon_stats(emoticon_path, sentiment_label, bg_color, sentiment, filtered_df, df):
            with open(emoticon_path, 'rb') as f:
                img_data = f.read()
                img_base64 = base64.b64encode(img_data).decode()

            sentiment_count = filtered_df[filtered_df['Sentiment'] == sentiment].shape[0]
            ...
            st.markdown(
                    ...
                    f"<h4 style='margin-top: -26px; padding-left: 10px;'>{sentiment_percentage:.2f}%</h4>"
                    "</div>"
                    "</div>"
                    "</div>",
                    unsafe_allow_html=True
        ...
        with emoticon1:
            display_emoticon_stats('assets/smile.png', 'Positive', 'CCFB5D', 'positive', filtered_df, df)
        with emoticon2:
            display_emoticon_stats('assets/neutral.png', 'Neutral', 'E5E4E2', 'neutral', filtered_df, df)
        with emoticon3:
            display_emoticon_stats('assets/sad.png', 'Negative', 'FF8674', 'negative', filtered_df, df)
    with body3:
        st.markdown(
        ...
        df_hashtag_mentions = df_hashtags.groupby('Hashtag')['Tweet'].count().reset_index()
        ...
        hashtag_names_short = [tag[:15] + '...' if len(tag) > 18 else tag for tag in df_top_hashtags['Hashtag']]
        # Create the pie chart
        fig = px.pie(df_top_hashtags, values='Mentions', names=hashtag_names_short,
                    hover_data=['Mentions'],
        ...
        fig.update_layout(
            ...
            margin=dict(t=10),
            legend={'font': {'size': 15}, 'title': {'text': 'Hashtags', 'font': {'size': 20}}},
            font=dict(size=15),
    ...
    for sentiment in sentiment_counts.index:
        top_tweets = filtered_df[filtered_df['Sentiment'] == sentiment].nlargest(10, 'Hashtags Count')
    ...
    for sentiment, color in sentiment_colors.items():
        filtered_sentiment = top_tweets_per_sentiment[top_tweets_per_sentiment['Sentiment'] == sentiment]
        trace = go.Bar(
    ...
    fig.update_layout(
        ...
        margin=dict(t=10),
        height=650,
        legend={'font': {'size': 15}, 'title': {'text': 'Sentiment', 'font': {'size': 20}}},
        xaxis={'tickfont': {'size': 15}, 'title': {'text': 'No of Records', 'font': {'size': 20}}},
        yaxis={'tickfont': {'size': 15}, 'title': {'text': 'Aspect', 'font': {'size': 20}}}
    )
